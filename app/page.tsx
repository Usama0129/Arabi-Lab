  ...
  const playArticleAudio = () => {
    ...
    function startPlayback() {
        if (activeArticle?.category === "会話" || (activeArticle?.level === "会話" && activeArticle.sentences)) {
          ...
          const textToRead = (activeArticle!.level === "中級" || activeArticle!.level === "上級" || activeArticle?.category === "物語") 
             ...
             <div className="bg-white p-1 rounded-lg shadow-sm hover:scale-105 transition-transform duration-300 border border-amber-400/30 cursor-pointer">
                <img 
                  ...
                  onError={(e) => {
                    ...
                    e.currentTarget.parentElement!.innerHTML = '<div class="w-10 h-10 bg-gradient-to-br from-emerald-600 to-emerald-800 rounded-lg flex items-center justify-center text-white text-xl font-bold">🇸🇦</div>';
                  }}
                />
             </div>
          ...
          <div className="flex items-center gap-2">
            {user ? (
                <div className="flex items-center gap-2 mr-2">
                    ...
                    <button onClick={handleLogout} className="flex items-center gap-2 bg-emerald-900 text-emerald-100 px-3 py-1 rounded-full text-xs font-bold hover:bg-red-900 transition border border-emerald-700">
                    {user.user_metadata?.avatar_url ? (
                        <img src={user.user_metadata.avatar_url} className="w-5 h-5 rounded-full border border-white/30" alt="icon" />
                    ) : (
                        <span>👤</span>
                    )}
                ...
                <button onClick={handleLogin} className="bg-white text-emerald-900 px-4 py-2 rounded-full font-bold text-xs shadow hover:bg-gray-100 transition mr-2 flex items-center gap-1">
                    <span className="text-blue-500 font-bold text-sm">G</span> ログイン
                </button>
            )}
      ...
      <main className="max-w-3xl mx-auto p-4 pb-20">
        {currentScreen === "mypage" && (
           <div className="animate-fade-in-up">
             ...
             {user && (
                 <div className="bg-white p-4 rounded-xl shadow-sm border border-emerald-100 mb-6 flex items-center gap-4" dir="ltr">
                    {user.user_metadata?.avatar_url ? (
                        <img src={user.user_metadata.avatar_url} className="w-12 h-12 rounded-full" alt="User" />
                    ) : <div className="w-12 h-12 bg-emerald-100 rounded-full flex items-center justify-center text-2xl">👤</div>}
                    <div>
                        <div className="flex items-center gap-2">
                           <p className="font-bold text-emerald-900">{user.user_metadata?.full_name || "ユーザー"}</p>
                           {isPremium ? <span className="text-xs bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full border border-amber-200">👑 有料会員</span> : <span className="text-xs bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full">無料会員</span>}
                        </div>
             ...
             <div className="grid grid-cols-3 gap-4 mb-8 text-center" dir="ltr">
        ...
        {currentScreen === "levels" && (
          <div className="text-center py-10 animate-fade-in-up">
            ...
            <div className="grid grid-cols-2 gap-4" dir="ltr">
              <LevelButton title="初級" subtitle="文法・短文" color="bg-emerald-50 border-emerald-200" icon="🌱" onClick={() => handleSelectLevel("初級")} />
              <LevelButton title="会話" subtitle="日常会話" color="bg-amber-50 border-amber-200" icon="💬" onClick={() => handleSelectLevel("会話")} />
              <LevelButton title="中級" subtitle="物語・文化" color="bg-blue-50 border-blue-200" icon="📖" onClick={() => handleSelectLevel("中級")} />
              <LevelButton title="上級" subtitle="ニュース" color="bg-purple-50 border-purple-200" icon="📰" onClick={() => handleSelectLevel("上級")} />
            </div>
          </div>
        )}
              ...
              {allArticles.filter(a => a.level === selectedLevel && (a.category === selectedCategory || (selectedLevel !== "初級" && selectedLevel !== "会話" && selectedLevel !== "中級" && selectedLevel !== "上級"))).map((article, index) => {
                  ...
                  return (
                    <div key={article.id} onClick={() => handleArticleClick(article, index)} className={`p-5 rounded-xl shadow-sm border flex justify-between items-center transition-all group cursor-pointer ${locked ? "bg-stone-100 border-stone-200" : "bg-white hover:shadow-md border-stone-100 hover:border-amber-300"}`}>
                        ...
                        {locked && (
                            <div className="bg-gray-200 w-8 h-8 rounded-full flex items-center justify-center text-lg shadow-inner" title="有料会員限定">🔒</div>
                        )}
                        <h3 className={`font-bold text-lg ${locked ? "text-gray-400" : "text-gray-800 group-hover:text-emerald-800 transition"}`}>
                            {selectedLevel === "初級" ? `問題 ${index + 1} (${article.title})` : article.title}
                        </h3>
                        </div>
                        {completedArticleIds.includes(article.id) ? 
                            <span className="text-emerald-600 text-xl bg-emerald-100 p-1 rounded-full">✓</span> : 
                            (locked ? <span className="text-gray-300 text-sm">Premium</span> : <span className="text-stone-300 text-xl group-hover:text-amber-500 transition">❮</span>)
                        }
                    </div>
                  );
              })}
                  ...
                  {grammarQuestions.length > 0 ? (
                    <>
                      <div className="text-center mb-10">
                        <span className="inline-block bg-purple-50 text-purple-700 border border-purple-200 px-4 py-1 rounded-full text-xs font-bold mb-6 tracking-wider">QUIZ {currentQuestionIndex + 1} / {grammarQuestions.length}</span>
                        <h2 className="text-xl font-bold mb-4 leading-relaxed text-gray-800">{grammarQuestions[currentQuestionIndex].text}</h2>
                      </div>
                      <div className="space-y-4 mb-8">
                        {grammarQuestions[currentQuestionIndex].options.map((option, idx) => (
                          <button key={idx} onClick={() => handleGrammarAnswer(idx)} disabled={grammarFeedback !== null} className={`w-full p-5 text-right rounded-xl border transition-all shadow-sm ${grammarFeedback ? (idx === grammarQuestions[currentQuestionIndex].correctIndex ? "bg-emerald-50 border-emerald-500 text-emerald-900" : "bg-gray-50 border-gray-200 opacity-50") : "bg-white border-stone-200 hover:border-purple-400 hover:bg-purple-50"}`}>
          ...
          <div className="max-w-xl mx-auto animate-fade-in-up pb-32">
             ...
             <div className="bg-white p-8 rounded-2xl shadow-xl mb-4 text-center border border-stone-100">
               ...
               <input 
                 ...
                 className="w-full p-4 text-2xl border-2 border-stone-200 rounded-xl focus:border-amber-400 focus:ring-4 focus:ring-amber-100 outline-none font-arabic text-center mb-6 transition-all" 
               />
             ...
             <div className="fixed bottom-0 left-0 w-full bg-gray-100 border-t border-gray-300 p-2 z-30 shadow-2xl">
               <div className="max-w-3xl mx-auto">
                 <div className="flex flex-wrap gap-1 justify-center mb-2" dir="rtl">
                   {ARABIC_KEYS.map((char) => (
                     <button key={char} onClick={() => handleKeyClick(char)} className="w-10 h-12 bg-white rounded shadow border-b-4 border-gray-200 active:border-b-0 active:translate-y-1 font-arabic text-xl hover:bg-gray-50 text-gray-800">{char}</button>
                   ))}
                 </div>
                 <div className="flex gap-2 justify-center">
                   <button onClick={handleSpace} className="flex-1 max-w-xs h-12 bg-white rounded shadow border-b-4 border-gray-200 active:border-b-0 active:translate-y-1 text-gray-600 font-bold">SPACE</button>
                   <button onClick={handleBackspace} className="w-20 h-12 bg-red-100 text-red-600 rounded shadow border-b-4 border-red-200 active:border-b-0 active:translate-y-1 font-bold">⌫</button>
                 </div>
               </div>
             </div>
          </div>
               ...
               <div className="space-y-3 mb-6">
                 {activeArticle.questions[currentQuestionIndex].options.map((option, idx) => {
                   ...
                   if (isQuizResultVisible) {
                     if (idx === activeArticle.questions[currentQuestionIndex].correctIndex) { btnClass = "bg-emerald-100 border-emerald-500 text-emerald-900 font-bold"; } 
                     else if (idx === quizSelectedOption) { btnClass = "bg-red-100 border-red-500 text-red-900"; } 
               ...
               {isQuizResultVisible && (
                 <div className="animate-fade-in-up">
                   <div className={`p-4 rounded-xl text-center mb-6 border ${quizSelectedOption === activeArticle.questions[currentQuestionIndex].correctIndex ? "bg-emerald-50 border-emerald-200 text-emerald-800" : "bg-red-50 border-red-200 text-red-800"}`}>
                     ...
                     <p className="text-sm opacity-90">{activeArticle.questions[currentQuestionIndex].explanation}</p>
                   </div>
        ...
        {/* Result Screen (With Translation List) */}
            ...
            {!isFlashcardMode ? (
              <>
                <div className="flex justify-between items-center mb-6">
                  <h2 className="text-2xl font-serif font-bold text-emerald-950">📒 My 単語帳</h2>
                  <div className="flex gap-2">
                    <button onClick={() => setIsAddingWord(!isAddingWord)} className="bg-emerald-100 text-emerald-800 px-4 py-2 rounded-full font-bold shadow-sm hover:bg-emerald-200 transition text-xs flex items-center gap-1">
                      {isAddingWord ? "✕ 閉じる" : "＋ 単語を追加"}
                    </button>
                    {savedVocab.length > 0 && 
                      <button onClick={() => { setFcIndex(0); setFcFlipped(false); setIsFlashcardMode(true); }} className="bg-amber-500 text-emerald-950 px-4 py-2 rounded-full font-bold shadow-lg hover:bg-amber-400 transition text-xs flex items-center gap-1">
                        <span>▶</span> 暗記モード
                      </button>
                    }
                  </div>
                ...
                {isAddingWord && (
                  <div className="bg-white p-5 rounded-2xl shadow-lg border-2 border-emerald-100 mb-8 animate-fade-in-up">
                    <h3 className="font-bold text-emerald-900 mb-4 text-sm">新しい単語を登録</h3>
                    <div className="flex flex-col gap-3">
                      <input 
                        type="text" 
                        dir="rtl" 
                        placeholder="アラビア語 (例: كِتَاب)" 
                        value={newArabic}
                        onChange={(e) => setNewArabic(e.target.value)}
                        className="w-full p-3 border border-stone-200 rounded-xl focus:border-emerald-500 outline-none font-arabic text-right bg-stone-50"
                      />
                      <input 
                        type="text" 
                        placeholder="日本語の意味 (例: 本)" 
                        value={newJapanese}
                        onChange={(e) => setNewJapanese(e.target.value)}
                        className="w-full p-3 border border-stone-200 rounded-xl focus:border-emerald-500 outline-none bg-stone-50"
                      />
                      <button 
                        onClick={handleAddCustomWord}
                        disabled={!newArabic || !newJapanese}
                        className={`w-full py-3 rounded-xl font-bold transition shadow-md ${(!newArabic || !newJapanese) ? "bg-gray-200 text-gray-400" : "bg-emerald-600 text-white hover:bg-emerald-700"}`}
                      >
                        保存する
                      </button>
                    </div>
                ...
                {savedVocab.length === 0 ? (
                  <div className="text-center py-24 text-gray-400 bg-white rounded-3xl border border-dashed border-stone-300">
                    単語が登録されていません<br/>
                    <span className="text-xs mt-2 block">レッスン中に単語をタップするか、<br/>右上のボタンから追加してください</span>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {savedVocab.map((v, i) => (
                      <div key={i} className="bg-white p-4 rounded-xl shadow-sm border border-stone-100 flex justify-between items-center group hover:border-emerald-300 transition">
                        <div>
                          <span className="font-bold text-xl block text-emerald-900 font-arabic">{v.word}</span>
                          <span className="text-gray-500 text-sm">{v.meaning}</span>
                        </div>
                        <div className="flex gap-2">
                          <button onClick={() => speakText(v.word)} className="text-stone-300 hover:text-emerald-500 p-2 transition">🔊</button>
                          <button onClick={() => deleteWord(v.word)} className="text-stone-300 hover:text-red-400 p-2 transition">✕</button>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <div className="max-w-md mx-auto">
                <div className="flex justify-between items-center mb-6">
                  <button onClick={() => setIsFlashcardMode(false)} className="text-gray-400 font-bold hover:text-red-500">✕ 終了</button>
                  <button onClick={() => setFcReverse(!fcReverse)} className="text-xs bg-stone-200 text-stone-700 px-3 py-1 rounded-full font-bold">モード: {fcReverse ? "日→アラ" : "アラ→日"}</button>
                </div>
                <div onClick={() => setFcFlipped(!fcFlipped)} className="bg-white h-80 rounded-3xl shadow-2xl flex flex-col items-center justify-center p-8 cursor-pointer border-b-8 border-stone-100 active:border-b-0 active:translate-y-2 transition-all relative">
                  <button onClick={(e) => { e.stopPropagation(); speakText(savedVocab[fcIndex].word); }} className="absolute top-6 right-6 text-stone-300 hover:text-emerald-600 text-2xl transition">🔊</button>
                  <p className="text-gray-300 text-xs font-bold tracking-widest mb-8 uppercase">TAP TO FLIP</p>
                  <h2 className={`font-bold text-center text-emerald-950 ${fcFlipped ? (fcReverse ? "text-5xl font-arabic" : "text-3xl") : (fcReverse ? "text-3xl" : "text-5xl font-arabic")}`}>
                    {!fcFlipped ? (fcReverse ? savedVocab[fcIndex].meaning : savedVocab[fcIndex].word) : (fcReverse ? savedVocab[fcIndex].word : savedVocab[fcIndex].meaning)}
                  </h2>
                </div>
                <div className="flex justify-between items-center mt-10 px-8" dir="ltr">
                  <button onClick={prevCard} className="bg-white w-14 h-14 rounded-full font-bold text-xl text-stone-400 shadow-md hover:scale-110 transition border border-stone-100">←</button>
                  <span className="font-bold text-stone-400 text-sm tracking-widest">{fcIndex + 1} / {savedVocab.length}</span>
                  <button onClick={nextCard} className="bg-emerald-600 text-white w-14 h-14 rounded-full font-bold text-xl shadow-lg shadow-emerald-200 hover:bg-emerald-700 hover:scale-110 transition">→</button>
                </div>
              </div>
            )}
          </div>
            ...
            <div className="relative z-10">
              ...
              <button 
                onClick={() => {
                   ...
                   alert("実際のアプリではここでStripe決済画面に移動します💰");
                }}
